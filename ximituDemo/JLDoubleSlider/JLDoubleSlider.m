...
-(void)handlerRowWithBlock:(ReturnTimeStrBlock)block {
...
{
    _minLabel.text = [NSString stringWithFormat:@"%.0f%@秒",_minNum + (_tatol * (num-20.0)),_unit];
    _currentMinValue = _minNum + (_tatol * (num-20.0));
    if (self.timeblock) {
        self.timeblock(_currentMinValue, _currentMaxValue);
    }
}

- (void)valueMaxChange:(CGFloat)num
{
    _maxLabel.text = [NSString stringWithFormat:@"%.0f%@秒",_minNum +(_tatol * (num-20.0)),_unit];
...
-(void)setMinNum:(CGFloat)minNum
{
    ...
    _minLabel.text = [NSString stringWithFormat:@"%.0f%@秒",minNum,_unit];
...
-(void)setMaxNum:(CGFloat)maxNum
{
    ...
    _maxLabel.text = [NSString stringWithFormat:@"%.0f%@秒",maxNum,_unit];
